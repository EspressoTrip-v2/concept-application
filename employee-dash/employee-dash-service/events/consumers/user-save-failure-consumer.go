...
func (c *UserSaveFailureConsumer) Listen() {
	...
	c.onFailure(err, logcodes.ERROR, "Failure to declare exchange", "task/employee-dash-service/events/user-save-failure-consumer.go:34")

	queue, err := c.rabbitChannel.QueueDeclare("", false, false, true, false, nil)
	c.onFailure(err, logcodes.ERROR, "Failure to declare queue", "task/employee-dash-service/events/user-save-failure-consumer.go:37")

	err = c.rabbitChannel.QueueBind(queue.Name, string(c.bindKey), string(c.exchangeName), false, nil)
	c.onFailure(err, logcodes.ERROR, "Failure to bind queue to exchange", "task/employee-dash-service/events/user-save-failure-consumer.go:40")

	messages, err := c.rabbitChannel.Consume(queue.Name, "", false, false, false, false, nil)
	c.onFailure(err, logcodes.ERROR, "Failure to listen on queue", "task/employee-dash-service/events/user-save-failure-consumer.go:43")
	...
	go func() {
		for d := range messages {
			...
			if !ok {
				localLogger.Log(logcodes.ERROR, "go routine error", "task/employee-dash-service/events/user-save-failure-consumer.go:51", "Error deleting employee")
				// Acknowledge message if employee does not exist as its already been deleted
				err := d.Ack(false)
				if err != nil {
					localLogger.Log(logcodes.ERROR, "go routine message acknowledge error", "task/employee-dash-service/events/user-save-failure-consumer.go:55",
			...
			err := d.Ack(false)
			if err != nil {
				localLogger.Log(logcodes.ERROR, "go routine message acknowledge error", "task/employee-dash-service/events/user-save-failure-consumer.go:62",
					fmt.Sprintf("Error acknowkledging message: %v", string(d.Body)))
			}
		}
	}()
...
func (c *UserSaveFailureConsumer) deleteEmployee(data []byte) bool {
	...
	ok := c.onFailure(err, logcodes.ERROR, "Failed to unmarshal json", "task/employee-dash-service/events/user-save-failure-consumer.go:73")
	if !ok {
		return ok
	}
	var employee models.Employee
	err = c.mongoClient.FindOneAndDeleteEmployee(context.TODO(), bson.D{{"email", employeePayload.Email}}, &employee)
	ok = c.onFailure(err, logcodes.ERROR, "Delete employee failed", "task/employee-dash-service/events/user-save-failure-consumer.go:79")
	if !ok {
		return ok
	}
	c.onSuccess(logcodes.DELETED, "Employee deleted", "task/employee-dash-service/events/user-save-failure-consumer.go:83",

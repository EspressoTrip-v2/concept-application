...
func (r *EmployeeDashRpcHandlers) LoginGoogleUser(ctx context.Context, user *employeePackage.GoogleGrpcUser) (*employeePackage.GrpcResponsePayload, error) {
	...
	ok := r.onFailure(err, logcodes.ERROR, "Employee not registered", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:35")
	if !ok {
		return nil, status.Errorf(codes.NotFound, fmt.Sprintf("Employee not registered: %v", err.Error()))
	}

	if (emp.SignInType != string(oath.GOOGLE) || user.GetSub() != emp.ProviderId) && emp.SignInType != string(oath.UNKNOWN) {
		err := errors.New("Invalid credentials")
		r.onFailure(err, logcodes.ERROR, "Invalid Google credentials", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:42")
		return nil, status.Errorf(codes.PermissionDenied, err.Error())
	}

	if emp.SignInType == string(oath.GITHUB) || emp.SignInType == string(oath.LOCAL) {
		err := errors.New("Not Authorized")
		r.onFailure(err, logcodes.ERROR, "Not Authorized", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:48")
		return nil, status.Errorf(codes.PermissionDenied, err.Error())
	}
	...
	if emp.SignInType == string(oath.UNKNOWN) {
		...
		if customError != nil {
			...
			r.onFailure(err, logcodes.ERROR, customError.Message[0], "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:56")
	...
	ok = r.onFailure(err, logcodes.ERROR, "Unable to encode JWT", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:70")
	if !ok {
...
func (r *EmployeeDashRpcHandlers) LoginGitHubUser(ctx context.Context, user *employeePackage.GitHubGrpcUser) (*employeePackage.GrpcResponsePayload, error) {
	...
	filter := bson.D{{"email", user.GetEmail()}}
	err := r.mongo.FindOneEmployee(ctx, filter, &emp)
	ok := r.onFailure(err, logcodes.ERROR, "Employee not registered", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:110")
	if !ok {
		return nil, status.Errorf(codes.NotFound, fmt.Sprintf("Employee not registered: %v", err.Error()))
	}

	if (emp.SignInType != string(oath.GITHUB) || strconv.Itoa(int(user.GetId())) != emp.ProviderId) && emp.SignInType != string(oath.UNKNOWN) {
		err := errors.New("Invalid credentials")
		r.onFailure(err, logcodes.ERROR, "Invalid Google credentials", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:117")
		return nil, status.Errorf(codes.PermissionDenied, err.Error())
	}

	if emp.SignInType == string(oath.GOOGLE) || emp.SignInType == string(oath.LOCAL) {
		err := errors.New("Not Authorized")
		r.onFailure(err, logcodes.ERROR, "Not Authorized", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:123")
		return nil, status.Errorf(codes.PermissionDenied, err.Error())
	}

	if emp.SignInType == string(oath.UNKNOWN) {
		rs, customError := GrpcClient().LoginGitHubUser(ctx, (*userPackage.GitHubGrpcUser)(user))
		if customError != nil {
			err := errors.New(customError.Message[0])
			r.onFailure(err, logcodes.ERROR, customError.Message[0], "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:131")
			return nil, status.Errorf(codes.Unknown, customError.Message[0])
		}
	...
	// Encode the JWT
	jwt, err := utils.EncodeJwt(&emp)
	ok = r.onFailure(err, logcodes.ERROR, "Unable to encode JWT", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:145")
	if !ok {
		return nil, status.Errorf(codes.Internal, fmt.Sprintf("Unable to encode JWT payload"))
	}
...
func (r *EmployeeDashRpcHandlers) LoginLocalUser(ctx context.Context, user *employeePackage.LocalGrpcUser) (*employeePackage.GrpcResponsePayload, error) {
	...
	if (emp.SignInType != string(oath.GITHUB) || emp.SignInType != string(oath.GOOGLE)) && emp.SignInType != string(oath.UNKNOWN) {
		err := errors.New("Invalid credentials")
		r.onFailure(err, logcodes.ERROR, "Invalid Google credentials", "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:117")
		return nil, status.Errorf(codes.PermissionDenied, err.Error())
	}
	...
	if emp.SignInType == string(oath.UNKNOWN) {
		rs, customError := GrpcClient().LoginLocalUser(ctx, (*userPackage.LocalGrpcUser)(user))
		if customError != nil {
			err := errors.New(customError.Message[0])
			r.onFailure(err, logcodes.ERROR, customError.Message[0], "employee-dash/employee-dash-service/services/grpc/employee-grpc-handlers.go:131")
			return nil, status.Errorf(codes.Unknown, customError.Message[0])

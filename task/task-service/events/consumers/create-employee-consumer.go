...
func (c *CreateEmployeeConsumer) Listen() {
	...
	c.onFailure(err, logcodes.ERROR, "Failure to declare queue", "task/task-service/events/create-employee-consumer.go:38")
...
func (c *CreateEmployeeConsumer) createEmployee(data []byte) {
	...
	if err != nil {
		c.onFailure(err, logcodes.ERROR, "Failed to unmarshal json", "task/task-service/events/create-employee-consumer.go:58")
	}
	...
	if err != nil {
		c.onFailure(err, logcodes.ERROR, "Insert employee failed", "task/task-service/events/create-employee-consumer.go:68")
	}
}
